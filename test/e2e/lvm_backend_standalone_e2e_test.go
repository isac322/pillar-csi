...
var _ = Describe("E33.4: LVM 백엔드 독립 E2E (Standalone)", Label("default-profile", "E33-standalone", "lvm", "standalone", "e33"), Ordered, func() {
	...
	It("Thin LV 생성 + 호스트에서 /dev/<vg>/<lv> 존재 확인", Label("default-profile", "E33-standalone"), func() {
	...
	It("Linear LV 생성 + ProvisionMode 오버라이드 검증", Label("default-profile", "E33-standalone"), func() {
		ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
		defer cancel()
	...
	It("LV 삭제 + 디바이스 소멸 + 재삭제 멱등", Label("default-profile", "E33-standalone"), func() {
		ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
		defer cancel()
	...
	It("LV 확장 후 lvs 및 blockdev --getsize64 반영", Label("default-profile", "E33-standalone"), func() {
		ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
		defer cancel()
	...
	It("VG 용량: total > 0, available ≤ total", Label("default-profile", "E33-standalone"), func() {
	...
	It("생성된 LV가 lvs에 포함됨 (ListVolumes 동등 동작)", Label("default-profile", "E33-standalone"), func() {
		ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
		defer cancel()
	...
	It("CreateVolume과 ListVolumes 모두 /dev/<vg>/<lv> 형식 일관", Label("default-profile", "E33-standalone"), func() {
		ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
		defer cancel()
