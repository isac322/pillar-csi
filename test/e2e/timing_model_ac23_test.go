...
//
// Validates that the full 437-TC suite can complete within the 2-minute wall-
// clock budget by:
//
//  1. Measuring actual per-TC isolation-scope latency (setup + teardown)
//     using real StartTestCase / Close calls — no mocks.
//
//  2. Computing the projected test-exec time for 437 TCs at the configured
...
const (
	...
	// (stageTestExec). The model projects that 437 TCs at 8 workers must finish
	// within this window.
...
// that the projected 437-TC suite time with 8 workers fits within the 45-second
// test-exec budget.
//
// Sub-AC 2.3 contract:
//   - measured average per-TC latency × 3 (overhead factor) × 437 TCs / 8 workers ≤ 45s
...
func TestAC23PerTCIsolationScopeLatencyProfile(t *testing.T) {
	...
	// and distribute 437 TCs across 8 parallel workers.
	//
	// projected = ceil(437 / 8) × avg × 3
...
// maxParallelProcs (8) workers is sufficient to execute all 437 default-profile
...
//     437 TCs complete in ceil(437/8) × 10ms = 55 × 10ms = 550ms ≪ 45s.
...
func TestAC23WorkerCountAchievesTestsBudget(t *testing.T) {
	...
	// but in-process TCs (Category A, the majority of 437) complete in < 1ms
...
//
//   - Minimum workers to meet the 45s test-exec budget (≥ ceil(437×10ms/45s) = 1)
...
func TestAC23ParallelWorkerCountIsOptimalForBudget(t *testing.T) {
	...
	// Minimum workers = ceil(437 × conservativePerTCMs / budgetMs).
...
// test-exec budget for the full 437-TC suite.
//
...
//
// Sub-AC 2.3 contract: time to process 437 TCs at ac23WorkerCount workers
...
func TestAC23ConcurrentThroughputMeetsTestsBudget(t *testing.T) {
	...
	// Project the sample throughput to the full 437-TC suite.
	// projected = elapsed × (437 / sampleSize)
	...
	t.Logf("  projected (437 TCs):  %s", projectedForFullSuite)
	t.Logf("  budget:               %s", budget)
...
func TestAC23BottleneckPhaseIdentification(t *testing.T) {
	...
	if avgSetup > phaseLatencyLimitMs*time.Millisecond {
		t.Errorf("AC23: avg setup latency %s exceeds %dms limit — "+
			"isolation scope creation is too slow for 437-TC budget; "+
	...
	if avgTeardown > phaseLatencyLimitMs*time.Millisecond {
		t.Errorf("AC23: avg teardown latency %s exceeds %dms limit — "+
			"isolation scope teardown is too slow for 437-TC budget; "+
