...
//
// Validates that the full 404-TC suite can complete within the 2-minute wall-
// clock budget by:
//
//  1. Measuring actual per-TC isolation-scope latency (setup + teardown)
//     using real StartTestCase / Close calls — no mocks.
//
//  2. Computing the projected test-exec time for 404 TCs at the configured
...
const (
	...
	// Sub-AC 2.3 budgets for 404 in-process TCs.
	ac23DefaultProfileCaseCount = 404
	...
	// (stageTestExec). The model projects that 404 TCs at 8 workers must finish
	// within this window.
...
// that the projected 404-TC suite time with 8 workers fits within the 45-second
// test-exec budget.
//
// Sub-AC 2.3 contract:
//   - measured average per-TC latency × 3 (overhead factor) × 404 TCs / 8 workers ≤ 45s
...
func TestAC23PerTCIsolationScopeLatencyProfile(t *testing.T) {
	...
	// and distribute 404 TCs across 8 parallel workers.
	//
	// projected = ceil(404 / 8) × avg × 3
...
// maxParallelProcs (8) workers is sufficient to execute all 404 default-profile
...
//     404 TCs complete in ceil(404/8) × 10ms = 51 × 10ms = 510ms ≪ 45s.
...
//
//   - Minimum workers to meet the 45s test-exec budget (≥ ceil(404×10ms/45s) = 1)
...
func TestAC23ParallelWorkerCountIsOptimalForBudget(t *testing.T) {
	...
	// Minimum workers = ceil(404 × conservativePerTCMs / budgetMs).
...
// test-exec budget for the full 404-TC suite.
//
...
//
// Sub-AC 2.3 contract: time to process 404 TCs at ac23WorkerCount workers
...
func TestAC23ConcurrentThroughputMeetsTestsBudget(t *testing.T) {
	...
	// Project the sample throughput to the full 404-TC suite.
	// projected = elapsed × (404 / sampleSize)
	...
	t.Logf("  projected (404 TCs):  %s", projectedForFullSuite)
	t.Logf("  budget:               %s", budget)
...
func TestAC23BottleneckPhaseIdentification(t *testing.T) {
	...
	if avgSetup > phaseLatencyLimitMs*time.Millisecond {
		t.Errorf("AC23: avg setup latency %s exceeds %dms limit — "+
			"isolation scope creation is too slow for 404-TC budget; "+
	...
	if avgTeardown > phaseLatencyLimitMs*time.Millisecond {
		t.Errorf("AC23: avg teardown latency %s exceeds %dms limit — "+
			"isolation scope teardown is too slow for 404-TC budget; "+
