...
A Linux NVMe host caches the identifiers of each namespace it scanned (UUID, NGUID, EUI-64, CSI). When it reconnects after the target went away, it compares the newly reported identifiers with the cached ones. On any difference it logs `identifiers changed for nsid N` and removes the namespace, so I/O fails and the filesystem on it shuts down, even though the reconnect itself succeeded within `ctrlLossTmo`. The kernel target (nvmet) assigns a random namespace UUID and subsystem serial every time the agent recreates the export, which it does after every storage-node reboot.

The agent therefore writes the identity explicitly before it enables the namespace and verifies it by read-back. The namespace UUID, the namespace NGUID, and the subsystem serial are derived deterministically from the subsystem NQN, which is itself derived from the volume ID. A volume gets the same identity after an agent restart, after a storage-node reboot, after the agent's state directory is lost, and when the controller re-drives the export. ZFS and LVM volumes follow the same rule. No CRD or RPC field is involved.

The agent never changes the identity of a namespace that is live (enabled), because the connected hosts have already cached it and nvmet cannot change the identity without disabling the namespace. Exports created by 0.2.0 and earlier carry kernel-random identifiers. When the upgraded agent reconciles such a live export, it keeps the live identity and records it under `/var/lib/pillar-csi/agent/nvmet-identity/`, the same hostPath that holds the fencing marks. The next re-creation after a reboot then reproduces that identity exactly. The record belongs to the volume's lifecycle and is deleted when the volume is unexported. After that, the derived identity applies. A 0.2.0 export that was lost before the upgraded agent reconciled it cannot be recovered: its random identity existed only in kernel memory. Its consumers see the identity change once, and their pods must be restarted to stage the volume again.
