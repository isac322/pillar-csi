...
// pillar-csi custom resources (PillarAgent, PillarStorageClass, PillarVolumeState, etc.).
package controller
...
const (
	// Finalizer added to every PillarAgent to prevent deletion
	// while PillarStore resources still reference it.
	pillarAgentFinalizer = "pillar-csi.bhyoo.com/pillar-agent-protection"
	...
	// PillarAgent to mark it as a pillar-csi storage node.
	storageNodeLabel = "pillar-csi.bhyoo.com/agent-node"

	// Requeue interval before re-checking whether blocking PillarStores have been removed.
...
//
// For PillarAgent the reconciler:
...
func (r *PillarAgentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Fetch the PillarAgent instance.
	...
	log.Info("Reconciling PillarAgent", "name", target.Name, "deletionTimestamp", target.DeletionTimestamp)
	...
	if !controllerutil.ContainsFinalizer(target, pillarAgentFinalizer) {
		log.Info("Adding finalizer to PillarAgent", "finalizer", pillarAgentFinalizer)
		controllerutil.AddFinalizer(target, pillarAgentFinalizer)
...
func (r *PillarAgentReconciler) reconcileNormal(
	...
	case target.Spec.External != nil:
		...
		log.Info("PillarAgent uses external address", "name", target.Name, "address", resolved)
		...
		meta.SetStatusCondition(&target.Status.Conditions, metav1.Condition{
			...
			Reason:             "ExternalMode",
			Message:            "PillarAgent uses an external address; NodeExists condition does not apply",
		})
	...
	default:
		...
		log.Info("PillarAgent has no nodeRef or external spec", "name", target.Name)
		...
		meta.SetStatusCondition(&target.Status.Conditions, metav1.Condition{
			...
			ObservedGeneration: target.Generation,
			Reason:             "MissingSpec",
			Message:            "PillarAgent spec is invalid: neither spec.nodeRef nor spec.external is set",
		})
...
func (r *PillarAgentReconciler) reconcileNodeRef(
	...
	if err != nil {
		if client.IgnoreNotFound(err) == nil {
			...
			meta.SetStatusCondition(&target.Status.Conditions, metav1.Condition{
				...
				Reason:             "NodeNotFound",
				Message: fmt.Sprintf(
					"PillarAgent is not ready: Node %q was not found in the cluster", nodeRef.Name,
				),
			})
			target.Status.ResolvedAddress = ""
			...
			if statusErr != nil {
				return ctrl.Result{}, fmt.Errorf("failed to update PillarAgent status: %w", statusErr)
			}
		...
		statusErr := r.Status().Update(ctx, target)
		if statusErr != nil {
			log.Error(statusErr, "Failed to update PillarAgent status after node lookup error")
		}
	...
	if resolveErr != nil {
		...
		meta.SetStatusCondition(&target.Status.Conditions, metav1.Condition{
			...
			Reason:             "AddressNotResolved",
			Message: fmt.Sprintf(
				"PillarAgent is not ready: no resolvable address on Node %q", nodeRef.Name,
			),
		})
		target.Status.ResolvedAddress = ""
	...
	log.Info("PillarAgent node address resolved",
...
func (r *PillarAgentReconciler) populateCapabilitiesStatus(
	ctx context.Context,
	target *pillarcsiv1alpha1.PillarAgent,
	address string,
) {
...
func (r *PillarAgentReconciler) reconcileDelete(
	...
	if err != nil {
		return ctrl.Result{}, fmt.Errorf("failed to list PillarStores: %w", err)
	}
	...
	log.Info("PillarAgent deletion unblocked; removing finalizer", "name", target.Name)

	controllerutil.RemoveFinalizer(target, pillarAgentFinalizer)
	err = r.Update(ctx, target)
	if err != nil {
		return ctrl.Result{}, fmt.Errorf("failed to remove finalizer from PillarAgent: %w", err)
	}
...
// referenced by target, but only if no other PillarAgent references the same
// node.  This prevents the agent DaemonSet from being evicted when one of
// several targets sharing a node is deleted.
func (r *PillarAgentReconciler) maybeRemoveStorageNodeLabel(
	ctx context.Context,
	target *pillarcsiv1alpha1.PillarAgent,
) error {
	log := logf.FromContext(ctx)
	...
	if hasOther {
		log.Info("Other PillarAgents still reference this node; keeping storage-node label",
...
// otherTargetOnNode returns true if any PillarAgent other than excludeName
...
func (r *PillarAgentReconciler) otherTargetOnNode(
	ctx context.Context, excludeName, nodeName string,
) (bool, error) {
	allTargets := &pillarcsiv1alpha1.PillarAgentList{}
	listErr := r.List(ctx, allTargets)
	if listErr != nil {
		return false, fmt.Errorf("list PillarAgents to check node %q references: %w", nodeName, listErr)
	}
...
// SetupWithManager registers the PillarAgent controller with the manager and
// configures watches:
//   - Node objects: re-enqueues any PillarAgent whose spec.nodeRef.name
//     matches the changed node so that NodeExists / resolvedAddress stay
//     current when nodes appear, change, or disappear.
//   - PillarStore objects: re-enqueues the PillarAgent named in the pool's
//     spec.agentRef so that a deletion-blocked target is promptly unblocked
//     when the last referencing pool is removed.
func (r *PillarAgentReconciler) SetupWithManager(mgr ctrl.Manager) error {
	// mapNodeToTargets returns reconcile Requests for every PillarAgent whose
	...
	mapNodeToTargets := func(ctx context.Context, obj client.Object) []reconcile.Request {
		...
		targetList := &pillarcsiv1alpha1.PillarAgentList{}
	...
	// mapPoolToTarget re-enqueues the PillarAgent referenced by a changed
	// PillarStore.  This ensures that when the last blocking pool is deleted the
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		// Re-enqueue a PillarAgent when any of its referencing PillarStores
