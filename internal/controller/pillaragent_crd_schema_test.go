...
// E21.4 — PillarAgent CRD OpenAPI schema validation tests.
//
...
// api/v1alpha1/pillaragent_types.go:
//
...
//
// All tests attempt to create a PillarAgent that violates one constraint and
...
var _ = Describe("PillarAgent CRD Schema Validation", func() {
	...
	// ── E21.4 TC-163 — TestCRDSchema_PillarAgent_NodeRefName_Empty ──────────
	...
	It("TC-163: Should reject PillarAgent with empty spec.nodeRef.name (MinLength=1 violation)", func() {
		const objName = "e214-target-empty-noderef-name"
		By("submitting a PillarAgent whose spec.nodeRef.name is an empty string")
	...
	// ── E21.4 TC-164 — TestCRDSchema_PillarAgent_ExternalPort_Zero ──────────
	// spec.external.port is annotated +kubebuilder:validation:Minimum=1.
	// A port value of 0 should be rejected.
	It("TC-164: Should reject PillarAgent with spec.external.port=0 (Minimum=1 violation)", func() {
		const objName = "e214-target-port-zero"
		By("submitting a PillarAgent whose spec.external.port is 0")
		...
		Expect(err).To(HaveOccurred(),
			"API server should reject PillarAgent with spec.external.port=0")
		Expect(errors.IsInvalid(err)).To(BeTrue(),
	...
	// ── E21.4 TC-165 — TestCRDSchema_PillarAgent_ExternalAddress_Empty ──────
	// spec.external.address is annotated +kubebuilder:validation:MinLength=1.
	// An empty address string should be rejected.
	It("TC-165: Should reject PillarAgent with empty spec.external.address (MinLength=1 violation)", func() {
		const objName = "e214-target-empty-address"
		By("submitting a PillarAgent whose spec.external.address is an empty string")
	...
	// ── E21.4 TC-166 — TestCRDSchema_PillarAgent_NodeRefAddressType_Invalid ─
	...
	It("TC-166: Should reject PillarAgent with invalid spec.nodeRef.addressType (Enum violation)", func() {
		const objName = "e214-target-bad-addrtype"
		By("submitting a PillarAgent with spec.nodeRef.addressType=Hostname (not in enum)")
		...
		Expect(err).To(HaveOccurred(),
			"API server should reject PillarAgent with addressType=Hostname (not in enum)")
		Expect(errors.IsInvalid(err)).To(BeTrue(),
