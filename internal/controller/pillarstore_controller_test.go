...
var _ = Describe("PillarStore Controller", func() {
	...
	// Helper: create a minimal PillarStore referencing targetName.
	createPool := func() {
	...
	// Helper: delete the PillarStore (ignoring not-found).
	deletePool := func() {
		resource := &pillarcsiv1alpha1.PillarStore{}
	...
	// Helper: create a PillarAgent with name targetName and the given Ready condition.
	createTarget := func(readyStatus *metav1.ConditionStatus, readyMsg string) {
		...
		if err != nil && errors.IsNotFound(err) {
			resource := &pillarcsiv1alpha1.PillarAgent{
				ObjectMeta: metav1.ObjectMeta{
					Name: targetName,
				},
	...
	// Helper: delete the PillarAgent (ignoring not-found).
	deleteTarget := func() {
		resource := &pillarcsiv1alpha1.PillarAgent{}
		if err := k8sClient.Get(bctx, types.NamespacedName{Name: targetName}, resource); err == nil {
			controllerutil.RemoveFinalizer(resource, pillarAgentFinalizer)
			Expect(k8sClient.Update(bctx, resource)).To(Succeed())
			Expect(k8sClient.Delete(bctx, resource)).To(Succeed())
		}
	}

	// fetchPool fetches the current PillarStore from the API server.
	fetchPool := func() *pillarcsiv1alpha1.PillarStore {
		fetched := &pillarcsiv1alpha1.PillarStore{}
		Expect(k8sClient.Get(bctx, poolNamespacedName, fetched)).To(Succeed())
		return fetched
	}

	// findCondition returns the named condition from a PillarStore, or nil.
	findCondition := func(pool *pillarcsiv1alpha1.PillarStore, condType string) *metav1.Condition {
	...
	Context("Finalizer management", func() {
		...
		It("should add the pool-protection finalizer on first reconcile", func() {
			...
			Expect(controllerutil.ContainsFinalizer(fetched, pillarStoreFinalizer)).To(BeTrue(),
				"finalizer %q should be present after first reconcile", pillarStoreFinalizer)
		})
	...
	Context("TargetReady condition — target does not exist", func() {
		...
		It("should set TargetReady=False with reason TargetNotFound when PillarAgent is absent", func() {
	...
	Context("TargetReady condition — target exists and is Ready", func() {
		...
		It("should set TargetReady=True when the referenced PillarAgent has Ready=True", func() {
			...
			Expect(cond.Reason).To(Equal("AgentReady"))
	...
	Context("PoolDiscovered condition — target ready with discovered pools", func() {
		...
		It("should set Ready=True when TargetReady, PoolDiscovered, and BackendSupported are all True", func() {
			...
			Expect(findCondition(fetched, "AgentReady").Status).To(Equal(metav1.ConditionTrue))
	...
	Context("Capacity sync from DiscoveredPools", func() {
		...
		// createCapPool creates a PillarStore with a ZFS backend pointing at capZFSPool.
		createCapPool := func() {
		...
		// setCapTarget creates (or updates) the PillarAgent with Ready=True and
		...
		fetchCapPool := func() *pillarcsiv1alpha1.PillarStore {
			p := &pillarcsiv1alpha1.PillarStore{}
	...
	Context("Deletion blocking", func() {
		...
		// Helper: create a PillarStorageClass referencing this pool.
		...
		It("should block deletion and retain the finalizer while a PillarStorageClass references the pool", func() {
			createReferencingBinding()

			// Trigger deletion of the PillarStore.
			Expect(k8sClient.Delete(bctx, &pillarcsiv1alpha1.PillarStore{
			...
			fetched := fetchPool()
			Expect(controllerutil.ContainsFinalizer(fetched, pillarStoreFinalizer)).To(BeTrue(),
				"finalizer should be retained while the referencing PillarStorageClass exists")
		})

		It("should remove the finalizer and allow deletion once all referencing PillarStorageClasss are removed", func() {
			createReferencingBinding()
			...
			Expect(errors.IsNotFound(err)).To(BeTrue(),
				"PillarStore should be deleted once all referencing bindings are removed")
		})

		It("should remove the finalizer immediately when there are no referencing PillarStorageClasss", func() {
			...
			err = k8sClient.Get(bctx, poolNamespacedName, fetched)
			Expect(errors.IsNotFound(err)).To(BeTrue(),
				"PillarStore should be deleted when no bindings reference it")
		})
		...
		// ── E20.9.4 ──────────────────────────────────────────────────────────
		// TestPillarStoreController_DeletionBlocked_StatusMessageContainsAllBindingNames
		It("should list all referencing PillarStorageClass names in the DeletionBlocked status message", func() {
			const (
			...
			bindingA := &pillarcsiv1alpha1.PillarStorageClass{
			...
			bindingB := &pillarcsiv1alpha1.PillarStorageClass{
	...
	Context("CRD schema validation", func() {
		// E20.2.1
		// TestPillarStoreCRD_InvalidCreate_EmptyAgentRef
		It("should reject PillarStore with empty spec.agentRef (MinLength=1 violation)", func() {
			pool := &pillarcsiv1alpha1.PillarStore{
				ObjectMeta: metav1.ObjectMeta{
			...
			Expect(err).To(HaveOccurred(),
				"API server should reject PillarStore with empty spec.agentRef")
	...
	Context("Ready condition — BackendSupported=False causes Ready=False", func() {
		...
		// E20.7.3
		// TestPillarStoreController_Ready_False_BackendUnsupported
		It("should set Ready=False/ConditionsNotMet when BackendSupported=False even if TargetReady=True and PoolDiscovered=True", func() {
			...
			targetReadyCond := meta.FindStatusCondition(fetched.Status.Conditions, "AgentReady")
