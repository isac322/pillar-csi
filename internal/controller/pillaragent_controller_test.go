...
var _ = Describe("PillarAgent Controller", func() {
	...
	// Helper: create a minimal external PillarAgent (no nodeRef so no Node lookup required).
	createTarget := func() {
	...
	Context("Finalizer management", func() {
		...
		It("should add the pillar-agent-protection finalizer on first reconcile", func() {
			createTarget()
			...
			Expect(controllerutil.ContainsFinalizer(fetched, pillarAgentFinalizer)).To(BeTrue(),
				"finalizer %q should be present after first reconcile", pillarAgentFinalizer)
		})
	})
	...
	Context("Deletion blocking", func() {
		...
		// Helper: create a PillarStore that references this target.
		...
		It("should block deletion and retain the finalizer while a PillarStore references the target", func() {
			createReferencingPool()

			// Trigger deletion of the PillarAgent.
			Expect(k8sClient.Delete(bctx, &pillarcsiv1alpha1.PillarAgent{
			...
			Expect(k8sClient.Get(bctx, targetNamespacedName, fetched)).To(Succeed())
			Expect(controllerutil.ContainsFinalizer(fetched, pillarAgentFinalizer)).To(BeTrue(),
				"finalizer should be retained while the referencing PillarStore exists")
		})

		It("should remove the finalizer and allow deletion once all referencing PillarStores are removed", func() {
			createReferencingPool()
			...
			Expect(errors.IsNotFound(err)).To(BeTrue(),
				"PillarAgent should be deleted once all referencing pools are removed")
		})

		It("should remove the finalizer immediately when there are no referencing PillarStores", func() {
			...
			err = k8sClient.Get(bctx, targetNamespacedName, fetched)
			Expect(errors.IsNotFound(err)).To(BeTrue(),
				"PillarAgent should be deleted when no pools reference it")
		})

		// ── E19.7.4 ────────────────────────────────────────────────────────────
		// TestPillarAgentController_DeletionBlocked_MultiplePoolsRequireAllRemoved
		//
		// When two PillarStores both reference the same PillarAgent, deletion must
		...
		Context("E19.7.4 — multiple pools: deletion stays blocked until the last pool is removed", func() {
			...
			BeforeEach(func() {
				// Create the PillarAgent.
				multiTarget := &pillarcsiv1alpha1.PillarAgent{
				...
				// Create two PillarStores that both reference this target.
			...
			It("E19.7.4 TestPillarAgentController_DeletionBlocked_MultiplePoolsRequireAllRemoved", func() {
				// Step 1: Trigger deletion of the PillarAgent.
				Expect(k8sClient.Delete(bctx, &pillarcsiv1alpha1.PillarAgent{
					ObjectMeta: metav1.ObjectMeta{Name: multiTargetName},
				})).To(Succeed())
				...
				Expect(k8sClient.Get(bctx, multiTargetNN, t)).To(Succeed(),
					"PillarAgent should still exist while pools reference it")
				Expect(controllerutil.ContainsFinalizer(t, pillarAgentFinalizer)).To(BeTrue(),
				...
				t = &pillarcsiv1alpha1.PillarAgent{}
				Expect(k8sClient.Get(bctx, multiTargetNN, t)).To(Succeed(),
					"PillarAgent should still exist while pool B references it")
				Expect(controllerutil.ContainsFinalizer(t, pillarAgentFinalizer)).To(BeTrue(),
	...
	Context("NodeRef mode — node found and auto-labeling", func() {
		...
		BeforeEach(func() {
			// Create the node that the PillarAgent will reference.
			node := &corev1.Node{
		...
		It("should remove the storage-node label from the node on target deletion", func() {
			...
			Expect(node.Labels).NotTo(HaveKey(storageNodeLabel),
				"storage-node label should be removed when PillarAgent is deleted")
		})
	})
	...
	Context("AgentConnected with injected mock Dialer — healthy agent", func() {
		...
		var dialerReconciler *PillarAgentReconciler

		BeforeEach(func() {
			dialerReconciler = &PillarAgentReconciler{
				Client: k8sClient,
				Scheme: k8sClient.Scheme(),
	...
	Context("AgentConnected with injected mock Dialer — health check fails", func() {
		...
		var failReconciler *PillarAgentReconciler

		BeforeEach(func() {
			failReconciler = &PillarAgentReconciler{
				Client: k8sClient,
				Scheme: k8sClient.Scheme(),
	...
	Context("AgentConnected with injected mock Dialer — agent reports degraded health", func() {
		...
		var unhealthyReconciler *PillarAgentReconciler

		BeforeEach(func() {
			unhealthyReconciler = &PillarAgentReconciler{
				Client: k8sClient,
				Scheme: k8sClient.Scheme(),
	...
	Context("AgentConnected with mTLS mock Dialer — non-TLS transport error", func() {
		...
		var mtlsNetFailReconciler *PillarAgentReconciler

		BeforeEach(func() {
			mtlsNetFailReconciler = &PillarAgentReconciler{
				Client: k8sClient,
				Scheme: k8sClient.Scheme(),
				Dialer: &mockDialer{
					mtls: true,
	...
	Context("GetCapabilities — status population when agent is healthy", func() {
		...
		var capsReconciler *PillarAgentReconciler

		BeforeEach(func() {
			capsReconciler = &PillarAgentReconciler{
				Client: k8sClient,
				Scheme: k8sClient.Scheme(),
				Dialer: &mockDialer{
					healthy: true,
	...
	Context("GetCapabilities — status populated even when agent reports degraded health", func() {
		...
		var noCapReconciler *PillarAgentReconciler

		BeforeEach(func() {
			noCapReconciler = &PillarAgentReconciler{
				Client: k8sClient,
				Scheme: k8sClient.Scheme(),
