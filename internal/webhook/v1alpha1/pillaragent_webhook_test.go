...
// E21.2: PillarAgent webhook — immutable field update rejection tests.
//
// These tests validate that PillarAgentCustomValidator.ValidateUpdate() correctly
...
var _ = Describe("PillarAgent Webhook", func() {
	var (
		obj       *pillarcsiv1alpha1.PillarAgent
		oldObj    *pillarcsiv1alpha1.PillarAgent
		validator PillarAgentCustomValidator
	)
	...
	Context("When creating or updating PillarAgent under Validating Webhook", func() {

		// ── E21.2 — ID 151 ──────────────────────────────────────────────────────
		// TestPillarAgentWebhook_Update_DiscriminantSwitch_NodeToExternal
		...
		// ── E21.2 — ID 152 ──────────────────────────────────────────────────────
		// TestPillarAgentWebhook_Update_DiscriminantSwitch_ExternalToNode
		...
		// ── E21.2 — ID 153 ──────────────────────────────────────────────────────
		// TestPillarAgentWebhook_Update_NodeRefNameImmutable
		...
		// ── E21.2 — ID 154 ──────────────────────────────────────────────────────
		// TestPillarAgentWebhook_Update_ExternalAddressImmutable
		...
		// ── E21.2 — ID 155 ──────────────────────────────────────────────────────
		// TestPillarAgentWebhook_Update_ExternalPortImmutable
		...
		// ── E21.2 — ID 156 ──────────────────────────────────────────────────────
		// TestPillarAgentWebhook_Update_NodeRefNonIdentityFieldChange_OK
		...
		// ── E21.2 — ID 157 ──────────────────────────────────────────────────────
		// TestPillarAgentWebhook_Create_Valid
		It("Should allow valid PillarAgent creation (current ValidateCreate is no-op scaffolding)", func() {
			By("creating a PillarAgent with spec.nodeRef set to a valid node name")
			obj.Spec.NodeRef = &pillarcsiv1alpha1.NodeRefSpec{Name: "storage-node-1"}

			_, err := validator.ValidateCreate(ctx, obj)
			Expect(err).NotTo(HaveOccurred(),
				"Valid PillarAgent creation should be allowed")
		})

		// ── E19.1.1 ───────────────────────────────────────────────────────────
		// TestPillarAgentWebhook_ValidCreate_External
		// ValidateCreate should pass without error for a well-formed external spec.
		It("E19.1.1 TestPillarAgentWebhook_ValidCreate_External: should accept external spec with address+port", func() {
		...
		// ── E19.1.2 ───────────────────────────────────────────────────────────
		// TestPillarAgentWebhook_ValidCreate_NodeRef
		...
		It("E19.1.2 TestPillarAgentWebhook_ValidCreate_NodeRef: should accept nodeRef spec with node name set", func() {
...
// E19.2.2 TestPillarAgentCRD_InvalidCreate_ExternalPortTooLow
// E19.2.3 TestPillarAgentCRD_InvalidCreate_ExternalPortTooHigh
// E19.2.4 TestPillarAgentCRD_InvalidCreate_EmptyExternalAddress
//
...
var _ = Describe("PillarAgent CRD Schema Validation", func() {
	Context("E19.2 — k8sClient.Create should be rejected for invalid specs", func() {
		It("E19.2.1 TestPillarAgentCRD_InvalidCreate_EmptyNodeRefName: should reject spec.nodeRef.name=''", func() {
			By("creating a PillarAgent with spec.nodeRef.name='' (violates MinLength=1)")
			...
			Expect(err).To(HaveOccurred(),
				"API server should reject PillarAgent with empty spec.nodeRef.name")
		})

		It("E19.2.2 TestPillarAgentCRD_InvalidCreate_ExternalPortTooLow: should reject spec.external.port=0", func() {
			By("creating a PillarAgent with spec.external.port=0 (violates Minimum=1)")
			target := &pillarcsiv1alpha1.PillarAgent{
			...
			err := k8sClient.Create(ctx, target)
			Expect(err).To(HaveOccurred(),
				"API server should reject PillarAgent with spec.external.port=0 (below minimum 1)")
		})

		It("E19.2.3 TestPillarAgentCRD_InvalidCreate_ExternalPortTooHigh: should reject spec.external.port=65536", func() {
			By("creating a PillarAgent with spec.external.port=65536 (violates Maximum=65535)")
			target := &pillarcsiv1alpha1.PillarAgent{
			...
			err := k8sClient.Create(ctx, target)
			Expect(err).To(HaveOccurred(),
				"API server should reject PillarAgent with spec.external.port=65536 (above maximum 65535)")
		})

		It("E19.2.4 TestPillarAgentCRD_InvalidCreate_EmptyExternalAddress: should reject spec.external.address=''", func() {
			By("creating a PillarAgent with spec.external.address='' (violates MinLength=1)")
			target := &pillarcsiv1alpha1.PillarAgent{
			...
			err := k8sClient.Create(ctx, target)
			Expect(err).To(HaveOccurred(),
				"API server should reject PillarAgent with empty spec.external.address")
		})
	})
})
