...
- [PRD 갭 — 추가 TC](#prd-갭--추가-tc)
  ...
  - [C-NEW-15: NodePublishVolume — 부분 스테이징 및 재게시 엣지 케이스](#c-new-15-nodepublishvolume--부분-스테이징-및-재게시-엣지-케이스)
  - [C-NEW-16: NodeUnpublishVolume — 마운트 상태 손실 및 스테이징 미게시 엣지 케이스](#c-new-16-nodeunpublishvolume--마운트-상태-손실-및-스테이징-미게시-엣지-케이스)
  - [C-NEW-17: 노드 장애 시나리오](#c-new-17-노드-장애-시나리오)
  - [C-NEW-18: 할당량 시행 (Quota Enforcement)](#c-new-18-할당량-시행-quota-enforcement)
...
| ID | 테스트 함수 | 설명 | 사전 조건 | 단계 | 기대 결과 | 커버리지 |
|----|------------|------|----------|------|----------|---------|
| 3a | `TestCSIController_CreateVolume_MissingTargetParam` | `target` 파라미터 누락 시 InvalidArgument 반환 | ControllerServer 초기화; Parameters에서 `pillar-csi.bhyoo.com/target` 제거 | 1) target 파라미터 없는 CreateVolumeRequest 전송 | gRPC InvalidArgument; agent 호출 없음 | `CSI-C` |
| 3b | `TestCSIController_CreateVolume_MissingBackendTypeParam` | `backend-type` 파라미터 누락 시 InvalidArgument 반환 | ControllerServer 초기화; Parameters에서 `pillar-csi.bhyoo.com/backend-type` 제거 | 1) backend-type 파라미터 없는 CreateVolumeRequest 전송 | gRPC InvalidArgument; agent 호출 없음 | `CSI-C` |
| 3c | `TestCSIController_CreateVolume_MissingProtocolTypeParam` | `protocol-type` 파라미터 누락 시 InvalidArgument 반환 | ControllerServer 초기화; Parameters에서 `pillar-csi.bhyoo.com/protocol-type` 제거 | 1) protocol-type 파라미터 없는 CreateVolumeRequest 전송 | gRPC InvalidArgument; agent 호출 없음 | `CSI-C` |
...
> 이 섹션의 테스트 케이스는 E24 섹션으로 재분류되었습니다: E24.1 참조
...
> 이 섹션의 테스트 케이스는 E24.2 섹션으로 재분류되었습니다.

### E6.2 삭제 시 CRD 정리

> 이 섹션의 테스트 케이스는 E24.9 섹션으로 재분류되었습니다.
...
> 이 섹션의 테스트 케이스는 E24.3, E2.5 섹션으로 재분류되었습니다.
...
### C-NEW-15: NodePublishVolume — 부분 스테이징 및 재게시 엣지 케이스

> **Component test 근거:** NodePublishVolume이 StateNodeStagePartial 상태에서 게시를 거부하고, 게시 취소 후 재게시가 정상적으로 동작하는 상태 머신 전환 로직.

| ID | 테스트 함수 | 설명 | 사전 조건 | 단계 | 기대 결과 | 커버리지 |
|----|------------|------|----------|------|----------|---------|
| C-NEW-15-1 | `TestCSINode_NodePublishVolume_StagePartial_FailedPrecondition` | StateNodeStagePartial 상태에서 NodePublishVolume 호출 시 FailedPrecondition 반환 | NewNodeServerWithStateMachine; sm.ForceState(volumeID, StateNodeStagePartial) | 1) NodePublishVolumeRequest 전송 | gRPC FailedPrecondition; mounter.Mount 0회 호출 | `CSI-N`, `SM` |
| C-NEW-15-2 | `TestCSINode_NodePublishVolume_RepublishAfterUnpublish_Succeeds` | Unpublish 후 재게시 성공 (상태 머신 왕복) | SM: ControllerPublished → Stage → Publish → Unpublish | 1) NodeStageVolume; 2) NodePublishVolume (1회); 3) NodeUnpublishVolume; 4) NodePublishVolume (2회) | 4단계 모두 성공; 최종 SM 상태 = NodePublished | `CSI-N`, `SM` |

---

### C-NEW-16: NodeUnpublishVolume — 마운트 상태 손실 및 스테이징 미게시 엣지 케이스

> **Component test 근거:** 노드 재부팅으로 인한 마운트 테이블 손실(IsMounted=false) 시나리오와 게시 없이 스테이징만 된 볼륨에 NodeUnpublishVolume 호출 시 CSI spec §5.4.2 멱등성 요구사항 검증.

| ID | 테스트 함수 | 설명 | 사전 조건 | 단계 | 기대 결과 | 커버리지 |
|----|------------|------|----------|------|----------|---------|
| C-NEW-16-1 | `TestCSINode_NodeUnpublishVolume_LostMountState_TreatedAsSuccess` | IsMounted=false(마운트 상태 손실) 시 NodeUnpublishVolume 성공 반환 | mockMounter.isMountedFn → false (노드 크래시 시뮬레이션) | 1) NodeUnpublishVolumeRequest 전송 | 성공; Unmount 0회 호출 | `CSI-N`, `Mnt` |
| C-NEW-16-2 | `TestCSINode_NodeUnpublishVolume_VolumeStagedNotPublished_IdempotentSuccess` | NodeStaged 상태(미게시)에서 NodeUnpublishVolume 호출 시 멱등 성공 | SM: sm.ForceState(volumeID, StateNodeStaged) | 1) NodeUnpublishVolumeRequest 전송 | 성공(nil 오류); Unmount 0회 | `CSI-N`, `SM` |

---

### C-NEW-17: 노드 장애 시나리오

> **Component test 근거:** Agent 다이얼 실패 시 ControllerPublish/Unpublish의 Unavailable 오류 전파, 컨텍스트 취소 시 NodeStageVolume 오류 전파를 검증하는 장애 복구 오케스트레이션 로직.

| ID | 테스트 함수 | 설명 | 사전 조건 | 단계 | 기대 결과 | 커버리지 |
|----|------------|------|----------|------|----------|---------|
| C-NEW-17-1 | `TestCSIController_ControllerPublishVolume_AgentDialFails_Unavailable` | agent 다이얼 실패 시 ControllerPublishVolume이 Unavailable 반환 | newCSIControllerDialErrEnvWithK8s; dialErr=connection refused; CSINode annotation 존재 | 1) ControllerPublishVolumeRequest 전송 | gRPC Unavailable; AllowInitiator 0회 | `CSI-C`, `gRPC` |
| C-NEW-17-2 | `TestCSIController_ControllerUnpublishVolume_AgentDialFails_Unavailable` | agent 다이얼 실패 시 ControllerUnpublishVolume이 Unavailable 반환 | newCSIControllerDialErrEnvWithK8s; dialErr=connection refused; CSINode annotation 존재 | 1) ControllerUnpublishVolumeRequest 전송 | gRPC Unavailable; DenyInitiator 0회 | `CSI-C`, `gRPC` |
| C-NEW-17-3 | `TestCSINode_NodeStageVolume_ContextCancelled_ErrorPropagated` | 이미 취소된 컨텍스트로 NodeStageVolume 호출 시 오류 전파 | connector.connectFn → ctx.Err() 반환; ctx 취소 | 1) NodeStageVolumeRequest 전송 | 비-OK gRPC 상태; 컨텍스트 취소 오류 전파 | `CSI-N`, `Conn` |

---

### C-NEW-18: 할당량 시행 (Quota Enforcement)

> **Component test 근거:** agent.CreateVolume / agent.ExportVolume / agent.ExpandVolume이 ResourceExhausted를 반환할 때 CSI 계층이 동일한 gRPC 상태 코드를 CO에 전파하는지 검증. CO가 ResourceExhausted를 받으면 PVC에 "quota exceeded" 이벤트를 기록하고 재시도를 중단해야 함.

| ID | 테스트 함수 | 설명 | 사전 조건 | 단계 | 기대 결과 | 커버리지 |
|----|------------|------|----------|------|----------|---------|
| C-NEW-18-1 | `TestCSIController_CreateVolume_QuotaExceeded_ResourceExhausted` | agent.CreateVolume이 ResourceExhausted 반환 시 CSI CreateVolume도 ResourceExhausted | mockAgent.createVolumeFn → ResourceExhausted("pool quota exceeded") | 1) CreateVolumeRequest 전송 | gRPC ResourceExhausted; agent.ExportVolume 0회 | `CSI-C`, `Agent` |
| C-NEW-18-2 | `TestCSIController_CreateVolume_ExportVolumeQuotaExceeded_ResourceExhausted` | agent.ExportVolume이 ResourceExhausted 반환 시 CreateVolume도 ResourceExhausted | mockAgent.exportVolumeFn → ResourceExhausted("namespace table full") | 1) CreateVolumeRequest 전송 | gRPC ResourceExhausted; agent.CreateVolume 1회; agent.ExportVolume 1회 | `CSI-C`, `Agent` |
| C-NEW-18-3 | `TestCSIController_ControllerExpandVolume_QuotaExceeded_ResourceExhausted` | agent.ExpandVolume이 ResourceExhausted 반환 시 ControllerExpandVolume도 ResourceExhausted | mockAgent.expandVolumeFn → ResourceExhausted("expansion exceeds quota") | 1) ControllerExpandVolumeRequest(100GiB) 전송 | gRPC ResourceExhausted; agent.ExpandVolume 1회 | `CSI-C`, `Agent` |
