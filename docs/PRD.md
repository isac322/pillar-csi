...
- 볼륨 확장 (ExpandVolume). allowVolumeExpansion은 backend capability에서 자동 결정 + 사용자 오버라이드 가능
- AccessModes: RWO + RWOP + ROX
- volumeMode: Filesystem 지원
- fsType 선택 (ext4/xfs) + mkfs 옵션 오버라이드
- Helm chart
- K8s 내부 노드만
- gRPC 보안: Phase 1은 평문 (클러스터 내부 신뢰). TLS 옵션은 아키텍처에 포함하되 비활성

**미포함:** 스냅샷/클론, volumeMode: Block, 다른 backend/protocol, 외부 노드, CSI Topology, mTLS
