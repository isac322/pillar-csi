...
Suggested toolchain: Figma → export 1×. Verify by uploading to
`Settings → General → Social preview` and previewing on the GitHub repo page
**and** by pasting the repo URL into a Slack/Discord channel — the rendered
card must show the image, not the default avatar collage.

The source `.fig` file should land in this repo at
`docs/social-preview/pillar-csi-og.fig` so future tweaks are version-controlled.
The exported PNG can sit alongside it for traceability, but only the GitHub
upload is what social platforms actually fetch.
