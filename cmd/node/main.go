...
func (c *fabricsConnector) GetDevicePath(ctx context.Context, subsysNQN string) (string, error) {
	...
	if err == nil {
		...
		for _, entry := range entries {
			...
			if dp, ctrlErr := c.getDevicePathViaController(subsysPath); ctrlErr == nil && dp != "" {
				return dp, nil
			}
...
//
// subsysPath is the absolute path to the nvme-subsystem class directory for
...
func (c *fabricsConnector) getDevicePathViaController(subsysPath string) (string, error) {
	...
	for _, ctrlEntry := range ctrlEntries {
		...
		for _, nsEntry := range nsEntries {
			...
			dev := int((minor & 0xff) | ((major & 0xfff) << 8) |
				((minor &^ 0xff) << 12) | ((major &^ 0xfff) << 32)) //nolint:gosec
			mknodErr := syscall.Mknod(devPath, syscall.S_IFBLK|0o600, dev) //nolint:gosec
...
func (c *fabricsConnector) getDevicePathViaNvmeCli(ctx context.Context, subsysNQN string) (string, error) {
	...
	if err != nil {
		return "", nil // /dev unreadable; treat as not found
	}
...
func (c *fabricsConnector) nvmeIdCtrlSubNQN(ctx context.Context, devPath string) (string, error) {
...
func (c *fabricsConnector) isConnected(subsysNQN string) (bool, error) {
	...
	path, nvmeErr := c.getDevicePathViaNvmeCli(context.Background(), subsysNQN)
	if nvmeErr != nil {
...
// Mount delegates to the wrapped Mounter unchanged.
