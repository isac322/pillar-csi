...
// PillarAgentSpec defines the desired state of PillarAgent.
// Exactly one of nodeRef or external must be set (discriminated union).
type PillarAgentSpec struct {
...
type PillarAgentStatus struct {
	...
	// conditions represent the current state of the PillarAgent resource.
	//
...
// PillarAgent represents a storage agent instance that pillar-csi controller
// manages.  Users create PillarAgent resources; the controller reconciles gRPC
// connectivity and populates status fields.
type PillarAgent struct {
	...
	// spec defines the desired state of PillarAgent.
	// +required
	Spec PillarAgentSpec `json:"spec"`

	// status defines the observed state of PillarAgent.
	// +optional
	Status PillarAgentStatus `json:"status,omitzero"`
}

// +kubebuilder:object:root=true

// PillarAgentList contains a list of PillarAgent.
type PillarAgentList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitzero"`
	Items           []PillarAgent `json:"items"`
}

func init() {
	SchemeBuilder.Register(&PillarAgent{}, &PillarAgentList{})
}
